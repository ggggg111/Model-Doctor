...
def main():
    ...
    samples_per_class = int(len(high_confidence_data) / len(high_confidence_data.classes))

    model = load_model(model_name, num_classes, device)
    model.load_state_dict(torch.load(checkpoint_path))
    model.eval()

    diagnose_stage = DiagnoseStage(model, num_classes, samples_per_class, delta, device)
